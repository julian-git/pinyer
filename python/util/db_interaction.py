...
def db_castellers(db, colla_id_name):
    c = db.cursor()
    c.execute("""
select casteller.id, nickname, total_height, shoulder_height, shoulder_width, hip_height, stretched_height, axle_height, weight, strength, svg_rep, alt_text
from casteller
...
""", (colla_id_name,))
    ...
    ans = dict()
    for row in res:
        new_ans = dict(zip(('id', 'nickname', 'total_height', 'shoulder_height', 'shoulder_width', 'hip_height', 'stretched_height', 'axle_height', 'weight', 'strength', 'svg_rep', 'alt_text'), row))
